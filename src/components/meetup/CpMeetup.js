...
const CpMeetup = ({ mypageUser, mypageCategory }) => {
  ...
  const loadData = () => {
    if (!mypageUser) {
      // 맛집 탐방
      axios.get('https://port-0-eatmate-backend-mlem81pp426165a9.sel3.cloudtype.app/meetup/all')
  ...
  const loadMeetupData = () => {
    axios.get('https://port-0-eatmate-backend-mlem81pp426165a9.sel3.cloudtype.app/mymeetup', {
      params: { user_no }
    })
      .then(res => {
          ...
          <Link to={`/meetup/detail/${item.bm_no}`} key={item.bm_no}>
            <div className='item'>
              ...
              <div className='item-img'>
                <img src={`https://port-0-eatmate-backend-mlem81pp426165a9.sel3.cloudtype.app/uploads/meetup/${item.bm_img}`} alt="" />
              </div>
            </div>
          </Link>
        ))}
